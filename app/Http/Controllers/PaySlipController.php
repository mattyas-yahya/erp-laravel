...
class PaySlipController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($payslip_employee >= count($validatePaysilp)) {
            ...
            if (!empty($employeesSalary)) {
                return redirect()
                    ->route('payslip.index')
                    ->with('error', __('Please set employee salary.'));
            }
            ...
            return redirect()
                ->route('payslip.index')
                ->with('error', __('Payslip Already created.'));
        }
    }
    ...
    public function search_json(Request $request)
    {
        ...
        if (\Auth::user()->type == 'company') {
            $paylip_employee = PaySlip::select(['employees.id', 'employees.employee_id', 'employees.name', 'payslip_types.name as payroll_type', 'pay_slips.basic_salary', 'pay_slips.net_payble', 'pay_slips.id as pay_slip_id', 'pay_slips.status', 'employees.user_id', 'employees.branch_id', 'employees.department_id'])
            ...
            $paylip_employee = PaySlip::select(['employees.id', 'employees.employee_id', 'employees.name', 'payslip_types.name as payroll_type', 'pay_slips.basic_salary', 'pay_slips.net_payble', 'pay_slips.id as pay_slip_id', 'pay_slips.status', 'employees.user_id', 'employees.branch_id', 'employees.department_id'])
                ...
                ->where('employees.created_by', \Auth::user()->creatorId())
                ->where('employees.branch_id', \Auth::user()->employee->branch_id)
                ->get();
        }
        ...
        foreach ($paylip_employee as $employee) {
            if (Auth::user()->type == 'employee') {
                ...
                $namebranch = Branch::where('id', $employee->branch_id)->first();
                $namedepartment = Department::where('id', $employee->department_id)->first();
                $tmp = [];
                $tmp[] = $employee->id;
                $tmp[] = $employee->employee_id;
                $tmp[] = $employee->name;
                $tmp[] = !empty($employee->payroll_type) ? $employee->payroll_type : '-';
                $tmp[] = !empty($employee->basic_salary) ? \Auth::user()->priceFormat($employee->basic_salary) : '-';
                $tmp[] = !empty($employee->net_payble) ? \Auth::user()->priceFormat($employee->net_payble) : '-';
                if ($employee->status == 1) {
                    $tmp[] = 'Paid';
                ...
                $tmp[] = !empty($employee->pay_slip_id) ? $employee->pay_slip_id : 0;
                $tmp['url'] = route('employee.show', Crypt::encrypt($employee->id));
                $tmp['branch'] = $namebranch;
                $tmp['department'] = $namedepartment;
                $data[] = $tmp;
            }
        }
    ...
    public function send($id, $month)
    {
        ...
        if ($setings['payslip_sent'] == 1) {
            ...
            $template = EmailTemplate::where('name', 'LIKE', 'payslip_sent')->first();
            $content = EmailTemplateLang::where('parent_id', '=', $template->id)
                ->where('lang', 'LIKE', 'en')
                ->first();
            $content->content = Utility::replaceVariable($content->content, $payslipArr);
            $settings = env('MAIL_FROM_ADDRESS');
            Mail::to($employee->email)->send(new CommonEmailTemplate($content, $settings));

            return redirect()
                ->back()
                ->with('success', __('Payslip successfully sent.'));
