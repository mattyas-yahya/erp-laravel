...
class DashboardController extends Controller
{
     ...
     */
    public function account_dashboard_index($lang = ' ')
    {
        if(Auth::check())
        {
            ...
            else
            {
                ...
                else
                {
                    if (Auth::user()->can('manage production dashboard')) {
                        return redirect()->route('dashboard.production');
                    }
        ...
        else
        {
            if(!file_exists(storage_path() . "/installed")){
                ...
                if($settings['display_landing_page'] == 'on'){
                    ...
                    $solusi     = $this->solusiDashboard();
                    $industri   = $this->industriDashboard();
                    \App::setLocale($lang);
                    return view('layouts.landing', compact('settings', 'lang', 'solusi', 'industri'));
                }else{
    ...
    public function store(Request $request){
        ...
        try{
            ...
            Mail::send('email.demo', ['user' => $validated], function ($message) use ($validated) {
                $message->to('sales@intisolutions.id');
                $message->subject('Demo');
            });

            Mail::send('email.demo', ['user' => $validated], function ($message) use ($validated) {
                $message->to('pandawamultitech@gmail.com');
                $message->subject('Demo');
            });

            $notification = array(
                'message'   => 'Terima Kasih dengan content bisa ambil dari chatgpt :D',
            );

            return redirect()->route('home')->with($notification);

        } catch (\Throwable $e) {
            return redirect()->route('home')->with(['error' => 'Demo tidak berhasil ditambahkan! ' . $e->getMessage()]);
        }
    }

    private function solusiDashboard(){
        $data = [
            "Software ERP (Hash Core ERP)",
            "Sistem Manajemen Inventaris",
            "IT Inventory DJBC - Kawasan Berikat",
            "CRM - Prospek & Penjualan",
            ...
            "Solusi Management Sekolah",
            "Solusi Manajemen Konstruksi",
        ...
        if(Auth::check())
        {
            ...
            if(\Auth::user()->can('show hrm dashboard'))
            {
                ...
                if($user->type != 'client' && $user->type != 'company')
                {
                    ...
                    ->whereIn('branch_id', [Auth::user()->employee->branch_id, 0])
                    ->whereIn('department_id',  [Auth::user()->employee->department_id, 0])
                    ->get();
                    // source asli announcement
                    // ->leftjoin('announcement_employees', 'announcements.id', '=', 'announcement_employees.announcement_id')->where('announcement_employees.employee_id', '=', $emp->id)->orWhere(function ($q){
                    //     $q->where('announcements.department_id', '["0"]')->where('announcements.employee_id', '["0"]');
                    // })

                    $employees = Employee::get();
                    $meetings  = Meeting::orderBy('meetings.id', 'desc')->take(5)->leftjoin('meeting_employees', 'meetings.id', '=', 'meeting_employees.meeting_id')
                    ->where('meeting_employees.employee_id', '=', $emp->id)
                    ->get();
                    // source asli meeting
                    // >leftjoin('meeting_employees', 'meetings.id', '=', 'meeting_employees.meeting_id')
                    // ->where('meeting_employees.employee_id', '=', $emp->id)->orWhere(function ($q){
