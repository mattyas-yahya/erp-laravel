...
class BillController extends Controller
{
    ...
    public function create($vendorId)
    {
        if(\Auth::user()->can('create bill')) {
            ...
            return view('bill.create', compact('venders', 'bill_number', 'category', 'customFields', 'vendorId'));
        }
        else
        {
    ...
    public function store(Request $request)
    {
        ...
        $bill->order_number   = !empty($request->purchase_order_id) ? $request->purchase_order_id : 0;
        $bill->ref_invoice      = $request->ref_invoice;
        $bill->created_by     = \Auth::user()->creatorId();
        $bill->save();

        CustomField::saveData($bill, $request->customField);
        $details = PurchaseOrderDetail::where('po_id', $request->purchase_order_id)->get();

        foreach ($details as $detail) {
            $billProduct              = new BillProduct();
            $billProduct->bill_id     = $bill->id;
            $billProduct->product_id  = $detail->product_services_id;
            $billProduct->quantity    = $detail->qty ?? 0;
            $billProduct->discount    = $detail->discount ?? 0;
            $billProduct->price       = $detail->price ?? 0;
            $billProduct->description = $detail->description;

            $taxPpn = $detail->tax_ppn ? (0.11 * $detail->price * $detail->qty) : 0;
            $taxPph = $detail->tax_pph ? (0.003 * $detail->price * $detail->qty) : 0;
            $billProduct->tax = $taxPph + $taxPpn;
        ...
        // $type = 'bill';
        // $billId = $bill->id;
        // $description = $detail->qty . '  ' . __(' quantity purchase in bill') . ' ' . \Auth::user()->billNumberFormat($bill->bill_id);
        // Utility::addProductStock($detail->product_services_id, $detail->qty, $type, $description, $billId);
    ...
    public function show($ids)
    {
        ...
        $items       = PurchaseOrderDetail::with(['productService', 'productServiceUnit', 'goodsReceiptDetail'])
            ->where('po_id', $bill->order_number)
            ->get();

        $bill->customField = CustomField::getData($bill, 'bill');
        $customFields      = CustomField::where('created_by', '=', \Auth::user()->creatorId())->where('module', '=', 'bill')->get();

        return view('bill.view', compact('bill', 'vendor', 'billPayment', 'customFields', 'vendorId', 'items'));
    }
    ...
    public function edit($ids)
    {
        ...
        $purchaseOrders = PurchaseOrder::where('vender_id', $bill->vender_id)->get();

        $bill->customField = CustomField::getData($bill, 'bill');
        $customFields      = CustomField::where('created_by', '=', \Auth::user()->creatorId())->where('module', '=', 'bill')->get();

        return view('bill.edit', compact('venders', 'purchaseOrders', 'bill', 'bill_number', 'category', 'customFields', 'vendorId'));
    }
    ...
    public function update(Request $request, Bill $bill)
    {
        ...
        $bill->bill_date      = $request->bill_date;
        $bill->due_date       = $request->due_date;
        $bill->order_number   = $request->purchase_order_id;
        $bill->category_id    = $request->category_id;
        $bill->ref_invoice      = $request->ref_invoice;
        $bill->save();
    ...
    public function sent($id)
    {
        if (\Auth::user()->can('send bill')) {
            try {
                ...
                $vendorArr = [
                    'vender_bill_name' => $vender->name,
                    'vender_bill_number' =>$bill->bill,
                    'vender_bill_url' => $bill->url,
                    'vender_bill_id' => $bill->id,
                ];

                Mail::to($vender->email)->send(new VenderBillEmail($vendorArr));

                Utility::userBalance('vendor', $vender->id, $bill->getTotal(), 'credit');

                return redirect()->back()->with('success', __('Bill successfully sent.'));
            } catch(\Exception $e) {
                return redirect()->back()->with('error', __('Failed to send the bill email. Please try again later.'));
            }
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
...
//                $billPayment->save();
    ...
    public function venderBillSendMail(Request $request, $bill_id)
    {
        ...
        try {
            ...
            return redirect()->back()->with('error', __('Failed to send the bill email. Please try again later.') . ((isset($smtp_error)) ? '<br> <span class="text-danger">' . $smtp_error . '</span>' : ''));
        }
        ...
        foreach($bill->items as $product) {
            ...
            if (!empty($item->tax)) {
                foreach($taxes as $tax) {
                    $taxPrice      = Utility::taxRate($tax?->rate, $item?->price, $item?->quantity) ?? 0;
                    $totalTaxPrice += $taxPrice;
