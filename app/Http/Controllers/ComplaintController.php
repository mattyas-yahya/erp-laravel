...
class ComplaintController extends Controller
{
    ...
    public function create()
    {
        if(\Auth::user()->can('create complaint'))
        {
            ...
            return view('complaint.create', compact('branch', 'branch2'));
        }
        else
        {
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create complaint'))
        {
            ...
            $complaint->branch_id_from = $request->branch_id_from;
            $complaint->department_id_from = $request->department_id_from;
            $complaint->employee_id_from = $request->employee_id_from;
            $complaint->branch_id_against = $request->branch_id_against;
            $complaint->department_id_against = $request->department_id_against;
            $complaint->employee_id_against = $request->employee_id_against;
            ...
            if($setings['complaint_resent'] == 1)
            {
                ...
                $employee         = Employee::find($complaint->employee_id_against);
                $complaintArr = [
    ...
    public function edit($complaint)
    {
        ...
        if(\Auth::user()->can('edit complaint'))
        {
            ...
            {
                return view('complaint.edit', compact('complaint', 'current_employee', 'branch', 'departments', 'employees', 'branch2', 'departments2', 'employees2'));
            }
            else
            {
                return response()->json(['error' => __('Permission denied.')], 401);
