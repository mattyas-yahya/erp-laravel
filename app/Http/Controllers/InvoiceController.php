...
class InvoiceController extends Controller
{
    ...
    public function customer(Request $request)
    {
        ...
        $customerShippingAddress = CustomerShippingAddress::where('customer_id', $customer->id)->get();

        return view('invoice.customer_detail', compact('customer', 'customerShippingAddress'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $invoice->delivery_order_number = $request->delivery_order_number;
        $invoice->faktur_penjualan_number = $request->faktur_penjualan_number;
        $invoice->ref_number     = !empty($request->sales_order_id) ? $request->sales_order_id : 0;
        $invoice->note = $request->note;
        $invoice->created_by     = \Auth::user()->creatorId();
        $invoice->save();

        CustomField::saveData($invoice, $request->customField);
        $details = SalesOrderDetail::where('so_id', $request->sales_order_id)->get();

        foreach ($details as $detail) {
            $invoiceProduct              = new InvoiceProduct();
            $invoiceProduct->invoice_id     = $invoice->id;
            $invoiceProduct->product_id  = $detail->goods_receipt_details_id;
            $invoiceProduct->quantity    = $detail->qty;
            $invoiceProduct->discount    = $detail->discount ?? 0;
            $invoiceProduct->price       = $detail->sale_price;
            $invoiceProduct->description = $detail->description;

            $taxPpn = $detail->tax_ppn ? (0.11 * $detail->price * $detail->qty) : 0;
            $taxPph = $detail->tax_pph ? (0.003 * $detail->price * $detail->qty) : 0;
            $invoiceProduct->tax = $taxPph + $taxPpn;
    ...
    public function edit($ids)
    {
        ...
        $salesOrders = SalesOrder::find($invoice->ref_number)->get();

        $invoice->customField = CustomField::getData($invoice, 'invoice');
        $customFields         = CustomField::where('created_by', '=', \Auth::user()->creatorId())->where('module', '=', 'invoice')->get();

        return view('invoice.edit', compact('customers', 'invoice', 'invoice_number', 'category', 'customFields', 'salesOrders'));
    }
        ...
        if (\Auth::user()->can('show invoice')) {
            ...
            if (!empty($invoice->created_by) == \Auth::user()->creatorId()) {
                ...
                $items                = SalesOrderDetail::with('gr_from_so.productServiceUnit')
                    ->where('so_id', $invoice->ref_number)
                    ->get();
                $user                 = \Auth::user();
                $invoice->customField = CustomField::getData($invoice, 'invoice');
                $customFields         = CustomField::where('created_by', '=', \Auth::user()->creatorId())->where('module', '=', 'invoice')->get();
                ...
                return view('invoice.view', compact('invoice', 'customer', 'items', 'invoicePayment', 'customFields', 'user', 'customerShippingAddress'));
            } else {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        } else {
    ...
    public function sent($id)
    {
        ...
        if (\Auth::user()->can('send invoice')) {
            ...
            if ($setings['customer_invoice_sent'] == 1) {
                ...
                Mail::to($customer->email)->send(new InvoiceEmail($customerArr));

                return redirect()->back()
                    ->with('success', __('Invoice successfully sent.'));
            }
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    ...
    public function payment($invoice_id)
    {
        ...
        return view('invoice.payment', compact('customers', 'categories', 'accounts', 'invoice', 'paymentMethods'));
    }
    ...
    public function createPayment(Request $request, $invoice_id)
    {
        if (\Auth::user()->can('create payment invoice')) {
            ...
            $invoicePayment->payment_reference_number = $request->payment_reference_number;
            $invoicePayment->giro_number = $request->giro_number;
            $invoicePayment->giro_date = $request->giro_date;
            $invoicePayment->cheque_number = $request->cheque_number;
            $invoicePayment->cheque_date = $request->cheque_date;
            ...
            $revenue->category_id    = null;
            $revenue->payment_method = 0;
            $revenue->category_id    = ProductServiceCategory::where('created_by', '=', \Auth::user()->creatorId())->where('name', '=', 'Invoice')->first()->id ?? 0;
    ...
    public function invoice($invoice_id)
    {
        ...
        if ($invoice) {
            ...
            $invoice->items = SalesOrderDetail::with('gr_from_so.productServiceUnit')
                ->where('so_id', $invoice->ref_number)
                ->get();

            return view('invoice.templates.' . $settings['invoice_template'], compact('invoice', 'color', 'settings', 'customer', 'img', 'font_color', 'customFields', 'customerShippingAddress'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
