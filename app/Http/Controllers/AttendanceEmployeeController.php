...
class AttendanceEmployeeController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (\Auth::user()->can('manage attendance')) {           
            ...
            // dd(date('m', strtotime($request->month)));
            if (\Auth::user()->type != 'client' && \Auth::user()->type != 'company') {

                $emp = !empty(\Auth::user()->employee) ? \Auth::user()->employee->branch_id : 0;
                // dd($emp);
                $attendanceEmployee = AttendanceEmployee::where('branch_id', $emp);
                
                // by department
                if (!empty($request->department)) {
                    $attendanceEmployee->where('department_id', $request->department);
                }
                // by employee
                if (!empty($request->employee)) {
                    $attendanceEmployee->where('employee_id', $request->employee);
                }
                // by date
                ...
                if (!empty($request->branch)) {
                    $attendanceEmployee->where('branch_id', $request->branch);
                }
            ...
            return view('attendance.index', compact('attendanceEmployee', 'branch', 'department', 'getBranch'));
        } else {
    ...
    public function attendance(Request $request)
    {
        ...
        $employeeId      = !empty(\Auth::user()->employee) ? \Auth::user()->employee->id : 0;
        $employeebranchId      = !empty(\Auth::user()->employee) ? \Auth::user()->employee->branch_id : 0;
        $employeedepartmentId      = !empty(\Auth::user()->employee) ? \Auth::user()->employee->department_id : 0;
        $todayAttendance = AttendanceEmployee::where('employee_id', '=', $employeeId)->where('date', date('Y-m-d'))->first();
        if (empty($todayAttendance)) {
            // company_start_time = jam 9
            $startTime = Utility::getValByName('company_start_time');
            $endTime   = Utility::getValByName('company_end_time');
            ...
            // $late = $currentTime->diff($startTime)->format('%h:%i:%s');
            ...
            if (empty($checkDb)) {
                ...
                $employeeAttendance->branch_id   = $employeebranchId;
                $employeeAttendance->department_id   = $employeedepartmentId;
    ...
    public function bulkAttendance(Request $request)
    {
        if (\Auth::user()->can('create attendance')) {
            ...
            $getBranch = $request->branch;
            return view('attendance.bulk', compact('employees', 'branch', 'getBranch'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
            ...
            if (!empty($employees)) {
                foreach ($employees as $employee) {
                    ...
                    if ($request->$present == 'on') {
                        ...
                        $employeeAttendance->branch_id     = $request->branch_id;
                        $employeeAttendance->department_id = $request->department_id;
                        $employeeAttendance->date          = $request->date;
                        $employeeAttendance->status        = 'Present';
