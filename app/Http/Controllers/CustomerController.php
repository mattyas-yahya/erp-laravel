...
class CustomerController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $data = [];
            for ($i = 0; $i < count($request['shipping_name']); $i++) {
                if (empty(array_filter([
                    $request['shipping_name'][$i],
                    $request['shipping_country'][$i],
                    $request['shipping_state'][$i],
                    $request['shipping_phone'][$i],
                    $request['shipping_city'][$i],
                    $request['shipping_phone'][$i],
                    $request['shipping_zip'][$i],
                    $request['shipping_address'][$i],
                ...
                array_push($data, [
                    ...
                    'name' => $request['shipping_name'][$i],
                    'country' => $request['shipping_country'][$i],
                    'state' => $request['shipping_state'][$i],
                    'phone' => $request['shipping_phone'][$i],
                    'city' => $request['shipping_city'][$i],
                    'phone' => $request['shipping_phone'][$i],
                    'zip' => $request['shipping_zip'][$i],
                    'address' => $request['shipping_address'][$i],
                ]);
            }

            CustomerShippingAddress::insert($data);
    ...
    public function show($ids)
    {
        ...
        $customerShippingAddress = CustomerShippingAddress::where('customer_id', $id)->get();
    ...
    public function edit($id)
    {
        ...
        return view('customer.edit', compact('customer', 'customFields', 'customerShippingAddress'));
    }
    ...
    public function update(Request $request, Customer $customer)
    {
        ...
        CustomerShippingAddress::where('customer_id', $customer->id)->delete();
        $data = [];
        for ($i = 0; $i < count($request['shipping_name'] ?? []); $i++) {
            if (empty(array_filter([
                $request['shipping_name'][$i],
