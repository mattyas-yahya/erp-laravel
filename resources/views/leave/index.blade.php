                    ...
                    <table class="table datatable">
                            ...
                            @foreach ($leaves as $leave)
                                <tr>
                                    ...
                                    @endif
                                    <td>{{ !empty($leave->branch())?$leave->branch()->name:'-' }}</td>
                                    <td>{{ !empty($leave->department())?$leave->department()->name:'-' }}</td>
...
<script>
    ...
    $(document).on('change', 'select[name=branch_id]', function () {
        ...
        $("#department").html('');
        $.ajax({
            url: "{{url('leave/getdepartment/json')}}",
            type: "POST",
            data: {
                branch_id: idBranch,
                _token: '{{csrf_token()}}'
    ...
    $(document).on('change', '#department', function () {
        ...
        $("#employee").html('');
        $.ajax({
            url: "{{url('leave/getemployee/json')}}",
            type: "POST",
            data: {
