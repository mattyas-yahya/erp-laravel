@push('script-page')
    <script>
        ...
        $(document).ready(function() {
            ...
            var venderId = $('#vender').val();
            var url = $('#vender').data('url');
        ...
        $(document).on('change', '#vender', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    let options = `<select class="form-control select2" name="purchase_order_id" id="purchase_order_id"></select>`;
                    $('.purchase-order-select').html(options);

                    $('#purchase_order_id').append('<option value=""></option>');
                    data.forEach(item => {
                        $('#purchase_order_id').append('<option value="' + item.id + '">' + item.po_number +
                                '</option>');
                    });
        ...
        function setDetails(data) {
            ...
            data.forEach(item => {
                ...
                tr[0].querySelector('.price').textContent = `Rp ${parseFloat(item.price ?? 0).toLocaleString('id-ID', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`
                tr[0].querySelector('.total').textContent = `Rp ${parseFloat(quantityAmount * item.price).toLocaleString('id-ID', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`;

                tr[0].querySelector('.pph-amount').textContent = `Rp ${parseFloat(taxPph ?? 0).toLocaleString('id-ID', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`
                tr[0].querySelector('.ppn-amount').textContent = `Rp ${parseFloat(taxPpn ?? 0).toLocaleString('id-ID', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`
                tr[0].querySelector('.price-include').textContent = `Rp ${parseFloat(item?.goods_receipt_detail?.price_include ?? 0).toLocaleString('id-ID', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`
