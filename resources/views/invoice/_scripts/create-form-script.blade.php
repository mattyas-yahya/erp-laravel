    ...
    <script>
        ...
        $(document).on('change', '#customer', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    let options =
                        `<select class="form-control select2" name="sales_order_id" id="sales_order_id"></select>`;
                    $('.sales-order-select').html(options);

                    $('#sales_order_id').append('<option value=""></option>');
                    data.forEach(item => {
                        $('#sales_order_id').append('<option value="' + item.id + '">' + item
                            .so_number +
                            '</option>');
                    });
        ...
        function setDetails(data) {
            ...
            data.forEach(item => {
                ...
                let quantityAmount = item.gr_from_so?.product_service_unit?.name === 'Kg' ? item.gr_from_so.weight :
                    item.qty;
                ...
                tr[0].querySelector('.price').textContent =
                    `Rp ${parseFloat(item.sale_price ?? 0).toLocaleString('id-ID', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`

                tr[0].querySelector('.total').textContent =
                    `Rp ${parseFloat(quantityAmount * item.sale_price).toLocaleString('id-ID', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`;
