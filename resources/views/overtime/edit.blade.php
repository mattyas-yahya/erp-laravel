        ...
        <div class="form-group col-md-12">
            ...
            <select class="form-control select2" name="type" id="type" required>
                ...
                @foreach ($payrollStaffOvertimes as $payrollStaffOvertime)
                    <option value="{{ $payrollStaffOvertime->id }}" @if (old('type', $overtime->type) == $payrollStaffOvertime->id) selected @endif>{{ $payrollStaffOvertime->type }} ({{ __('Time Worked (hour)') }}: {{ floatval($payrollStaffOvertime->time_worked) }})</option>
                @endforeach
            </select>
        </div>
