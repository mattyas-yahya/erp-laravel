@foreach ($machines as $machine)
    <div class="tab-pane fade show" id="tab-machine-{{ $machine->id }}" role="tabpanel" aria-labelledby="pills-user-tab-1">
        <div class="table-responsive">
            <table class="table datatable{{ $loop->iteration }}">
                <thead>
                    @include('production.schedule._components.header-table')
                </thead>

                <tbody>
                    @forelse($schedules->where('machine_id', $machine->id) as $item)
                        <tr>
